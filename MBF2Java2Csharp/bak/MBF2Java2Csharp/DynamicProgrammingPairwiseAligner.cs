...
namespace MBF2Java2Csharp
{
    public abstract class DynamicProgrammingPairwiseAligner
    {
        ...
        /*******************************************************************************/
        // Todo: Saliya - optimization test cut offs based on profiling info
        /// <summary>
        ...
        unsafe protected void FillMatrixAffine() {
            ...
            int size = numberOfRows * numberOfCols;
            fSource = new sbyte[size];
            fixed (sbyte* fSourceElement = &fSource[0])
            {
                // Fill by rows
                short row, col;
                int cell;
...
//                    for (col = 1; col != numberOfCols; ++col, ++cell)
                    for (col = 1; col < numberOfCols; col++, cell++)
                    {
                        FillCellAffine(row, col, cell, fSourceElement);
                    }
                }
        ...
        /// <param name="cell">Cell number.</param>
        // Todo: Saliya - optimization test cut offs based on profiling info
        unsafe protected abstract void FillCellAffine(short row, short col, int cell, sbyte* fSourceElement);
        ...
        // Todo: Saliya - optimization test cut offs based on profiling info
        unsafe protected int SetCellValuesAffine(short row, short col, int cell, sbyte* fSourceElement) {
            int score;
...
//                    fSource[cell] = SourceDirection.Diagonal;
        ...
        protected abstract IList<AlignedData> Traceback();
