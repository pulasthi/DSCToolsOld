...
namespace Salsa.PlotViz.IO
{
    public class PlotVizDataFile
    {
        ...
        public static PlotVizDataFile Build(string indexFile, string clusterFile, string coordinateFiles)
        {
            var indexMap = BuildIndexMap(indexFile);
            var clusterMap = BuildClusterMap(clusterFile);
            var coordsMap = BuildCoordianteMap(coordinateFiles);
            ...
            foreach (var item in coordsMap.Keys)
            {
                PlotVizDataFilePoint point = new PlotVizDataFilePoint();
                point.Key = item;
                point.Coordinate = coordsMap[item];
            ...
            using (StreamReader reader = File.OpenText(fileName))
            {
                while (reader.EndOfStream == false)
                {
                    ...
                    if (string.IsNullOrEmpty(line) == false)
                    {
                        ...
                        coordianteMap.Add(Convert.ToInt32(fields[0]), coords);
                    }
                }
            }
